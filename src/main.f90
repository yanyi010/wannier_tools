...
!--------+--------+--------+--------+--------+--------+--------+------!
  ...
  program main
   ...
   if (BulkBand_cube_calc) then
     ...
     if (Floquet_band_calc)then
      if(cpuid.eq.0)write(stdout, *)' '
      if(cpuid.eq.0)write(stdout, *)'>> Start of calculating the floquet band structure'
      call now(time_start)
      call floquet_band
      call now(time_end)
      call print_time_cost(time_start, time_end, 'Floquet_band_calc')
      if(cpuid.eq.0)write(stdout, *)'End of calculating the floquet band structure'
   endif
